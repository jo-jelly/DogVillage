...
//   $("#nameChk").click(function(){
      $.ajax({
    	 	 url: '/aajjj/mypage/Mypage_deadReg',
             type: 'POST',
...
<body>
<form action="/projectMenu/mypage/mypage_deadReg" method="post" enctype="multipart/form-data" name="frm">
 <aside>
	<div class="list-group">
	  ...
	  <a href="mypage_qna" class="list-group-item list-group-item-action">1:1문의</a>
...
<article>
	...
	<h4>회원탈퇴 안내</h4></br>
		...
		<p>고객님께서 회원 탈퇴를 원하신다니 저희 캣팡의 서비스가 많이 부족하고 미흡했나 봅니다.</br>
				불편하셨던 점이나 불만사항을 알려주시면 적극 반영해서 고객님의 불편함을 해결해 드리도록 노력하겠습니다.</br>
				회원탈퇴 시 도그팡 서비스 이용이 불가합니다.</br>
				거래정보가 있는 경우, 전자상거래 등에서의 소비자 보호에 관한 법률에 따라 계약 또는 청약철회에 관한 기록, 대금결제 및 재화 등의 공급에 관한 기록은 5년동안 보존됩니다.</br>
				미사용 쿠폰은 탈퇴와 함께 삭제되며 재발급되지 않습니다.</br>
				보유하셨던 포인트는 탈퇴와 함께 삭제되며 환불되지 않습니다.</br>
				회원탈퇴 후 등록하신 상품문의 및 후기, 댓글은 삭제되지 않으며, 회원정보 삭제로 인해 작성자 본인을 확인할 수 없어 편집 및 삭제처리가 원천적으로 불가능 합니다.</br>
				이미 결제가 완료된 건은 탈퇴로 취소되지 않습니다.
		</p>
		</br>
		
		비밀번호 :
		<input type="password" name="ppw" id="pw"/><br>		
		
		<button class="nameChk" type="button" id="nameChk" onclick="fn_pwChk();">중복확인</button>
		...
		<input type="button" value="회원탈퇴" id="submitChk"  />
	</div>
</article>
</c:forEach>
